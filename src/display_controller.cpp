...
void DisplayController::repaint(bool fullRepaint, DisplayRenderPayload* data) {
    ...
    if (fullRepaint) repaintCounter = 0;
    if (fullRepaint || repaintCounter++ % N_UPDATES_BETWEEN_FULL_REPAINTS == 0) {
        drawBackground(data);
    }
    drawStatusBar(data);
    drawGauges(
    ...
    Serial.print((micros() - timestamp) / 1000.0f, 1);
    Serial.println("ms");
...
void DisplayController::drawGauges(
    ...
    display.setPartialWindow(0, 13, 105, 51);
    display.firstPage();
    do {
        unsigned long timestamp = micros();
        // gauges - labels
...
void DisplayController::drawStatusBar(DisplayRenderPayload* data) {
    ...
    display.setPartialWindow(0, 0, display.width(), Font_04b03b.yAdvance);
    display.firstPage();
    do {
        // sd card
        // sd card - icon
...
void DisplayController::drawAllStats(DisplayRenderPayload* data) {
    ...
    display.setPartialWindow(statX, statY, 107, 43);
    display.firstPage();
    do {
        unsigned long timestamp = micros();
...
void DisplayController::drawCurrentValues(DisplayRenderPayload* data, const float currentTemp, const char unitSymbol) {
    int16_t tbx, tby; uint16_t tbw, tbh;
    char buf[5];
    display.setPartialWindow(106, 11, 34, 56);
    display.firstPage();
    do {
        unsigned long timestamp = micros();
        // current values
...
void DisplayController::drawHistoryGraph(DisplayRenderPayload* data, const char unitSymbol) {
    int16_t tbx, tby; uint16_t tbw, tbh;
    char buf[5];
    display.setPartialWindow(0, 64, 250, 58);
    display.firstPage();
    do {
        unsigned long timestamp = micros();
        // graph - lines
